...
const transactions_info_columns = [
	...
	},
	{
		...
		dataIndex: 'key_id',
		render: text => (
			<a href={`#/ecosystem/1/member/${text}?state=income`}>{text}</a>
		)
	}
];
...
class BlockDetail extends React.Component {
    ...
    render() {
    	...
    	if (
    		!loading &&
            Block_header.length === 0 &&
            Block_info.length === 0 &&
            Transactions_info.length === 0
    	) {
